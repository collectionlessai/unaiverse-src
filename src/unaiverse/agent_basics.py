...
class AgentBasics:
    ...
    def __init__(self,
        ...
        self.behav: HybridStateMachine  # HSM that describes the agent behavior in the private/world net
        self.behav_lone_wolf: HybridStateMachine  # HSM that describes the agent behavior in public net
        ...
        self.compat_in_streams: list[set] = []  # Streams compatible with the processor input (dynamically set)
        self.compat_out_streams: list[set] = []  # Streams compatible with the processor output (dynamically set)
        ...
        self.node_ask_to_get_in_touch_fcn: Callable | None = None
        self.node_purge_fcn: Callable | None = None
        self.node_agents_waiting = None
        self.node_identity_dir = ''

        # Utilities (keep the "__" prefix, or they will be cleared on world leaving!)
        ...
        self.compat_in_streams = [set() for _ in range(len(self.proc_inputs))] \
            if self.proc_inputs is not None else []
        self.compat_out_streams = [set() for _ in range(len(self.proc_outputs))] \
            if self.proc_outputs is not None else []

        # Interaction Manager, stdin/stdout
        assert isinstance(self.proc_optional_inputs, list)
        self.im = InteractionManager(self, max_interactions=Custom.MAX_INTERACTIONS)
        self.stdin = StreamProxy(replace_none_samples_with=[item["default_value"] if item["has_default"] else None
        ...
        if hasattr(self, "process"):  # Trick to distinguish if this is an Agent or a World (both sons of this class)
            ...
            # Public HSM: safe way to load a file packed in a pip package
            ...
            json_file = utils_path.joinpath("public.json")
            file = json_file.open()
            self.behav_lone_wolf.load(file)
            file.close()
            self.set_policy_filter(self.policy_filter_lone_wolf, public=True)

            # PrivatE HSM: Setting an empty HSM as default is not provided (private/world)
    ...
    def set_node_info(self, conn: NodeConn, profile: NodeProfile,
        ...
        self.node_conn = conn
        self.node_profile = profile
        self.node_name = profile.get_static_profile()['node_name']
        self.node_ask_to_get_in_touch_fcn = ask_to_get_in_touch_fcn
        self.node_purge_fcn = purge_fcn
        self.node_identity_dir = node_identity_dir
        self.node_agents_waiting = agents_waiting

        # Checking if human, marking the node
        assert self.proc is None or isinstance(self.proc, ModuleWrapper)  # It is None in worlds!
        if self.proc is not None and isinstance(self.proc.module, HumanModule):
            self.node_profile.get_static_profile()["node_type"] = self.HUMAN
    ...
    def get_role(self, agent: str) -> str | None:
        role_int = self.node_conn.get_role(agent)
    ...
    def get_agents_by_role(self, role: str | list[str], handshake_completed: bool = True):
        ...
        for role_int in searched_roles_int:
            _, _found_connected_peer_ids = self.node_conn.find_addrs_by_role(role_int, return_peer_ids_too=True)
    ...
    def get_current_role(self, return_int: bool = False, ignore_base_role: bool = True) -> str | int | None:
        ...
        """
        if self.in_world():
            role_str = self.node_profile.get_dynamic_profile()['connections']['role']
            if ignore_base_role:
        ...
        for (net_hash, name) in to_remove:
            ...
            if DataProps.is_pubsub_from_net_hash(net_hash):
                if peer_id != "<private_peer_id>" and peer_id != "<public_peer_id>":
                    if not (await self.node_conn.unsubscribe(peer_id, channel=net_hash)):
    ...
    async def add_compatible_streams(self, peer_id: str,
        ...
        if add_all:
            ...
            assert self.proc_opts is not None
        ...
        for net_hash in net_hashes_to_subscribe:
            ...
            if not (await self.node_conn.subscribe(peer_id, channel=net_hash)):
    ...
    def update_streams_in_profile(self):
        ...
        dynamic_profile = self.node_profile.get_dynamic_profile()
    ...
    async def send_profile_to_all(self):
        ...
        for peer_id in agents:
            ...
            if not (await self.node_conn.send(peer_id, channel_trail=None,
                                              content=self.node_profile.get_all_profile(),
    ...
    async def behave(self):
        ...
        for interaction in completed:
            if (interaction.requester is not None and not interaction.volatile and
                ...
                ret = await self.node_conn.send(interaction.requester, channel_trail=None,
                ...
                if not ret:
                    ...
                    assert self.node_purge_fcn is not None
                    await self.node_purge_fcn(interaction.requester)
    ...
    def get_peer_ids(self) -> tuple[str, str]:
        ...
        if self.node_profile is not None:
            dynamic_profile = self.node_profile.get_dynamic_profile()
    ...
    def is_human(self):
        ...
        """
        if self.node_profile is not None:
            return self.node_profile.get_static_profile()["node_type"] == self.HUMAN
        else:
            return False
    ...
    def in_world(self):
        ...
        """
        if self.node_profile is not None:
            return self.ROLE_STR_TO_BITS[self.node_profile.get_dynamic_profile()['connections']['role']] & 1 == 1
        else:
            return False
    ...
    def get_stream_sample(self, net_hash: str,
        ...
        if sample_dict is None:
            log.error(f"Invalid sample (expected a dictionary, got None)")
            return added_data
        ...
        if net_hash in self.known_streams:
            for name, data_and_tag_and_uuid in sample_dict.items():
                ...
                if not skip:
                    ...
                    has_this_data_tag = self.known_streams[net_hash][name].get_tag(data_uuid)
                    if has_data:
                        assert data_tag is not None
                        assert has_this_data_tag is not None
                        if data_tag <= has_this_data_tag:
                            skip = True
    ...
    def reset_agent_status_attrs(self):
        ...
        for attr_name in dir(self):
            if (attr_name.startswith("_") and not attr_name.startswith("__") and not attr_name.startswith("_Agent")
                    and not attr_name.startswith("_WAgent") and not attr_name.startswith("_AgentBasics")):
                try:
                    value = getattr(self, attr_name)
                    if isinstance(value, list):
    ...
    def save(self, where: str = "") -> bool:
        ...
        """
        if not (isinstance(self.proc, ModuleWrapper) and isinstance(self.proc.module, torch.nn.Module)):
            return False

        if where == '':
            if self.node_identity_dir is None or len(self.node_identity_dir) == 0:
                return False
            where = os.path.join(self.node_identity_dir, "agent_state")  # Default save path
        ...
        if self.proc is not None and self.proc_updated_since_last_save:
            ...
            proc: ModuleWrapper = self.proc
            assert isinstance(proc.module, torch.nn.Module)

            pt_final = os.path.join(where, f"{self.node_name}.pt")
            pt_tmp = pt_final + ".tmp"
            try:
                checkpoint = {
                    'model_state_dict': proc.module.state_dict(),
                }

                # If your agent has an optimizer, save its state too
                assert self.proc_opts is not None
                if self.proc_opts.get('optimizer', None) is not None:
        ...
        # Save Agent State
        pkl_final = os.path.join(where, f"{self.node_name}.pkl")
    ...
    def load(self, where: str = "") -> bool:
        ...
        pkl_path = os.path.join(where, f"{self.node_name}.pkl")
        ...
        pt_path = os.path.join(where, f"{self.node_name}.pt")
        ...
        if load_proc:
            try:
                ...
                if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
                    proc.module.load_state_dict(checkpoint['model_state_dict'])
                    ...
                    proc.module.load_state_dict(checkpoint)
            except Exception as e:
    ...
    async def _send(self, interaction: Interaction | None = None,
        ...
        for _peer_id in interaction.target:
            ret = await self.node_conn.send(_peer_id, channel_trail=None,
    ...
    def __str__(self):
        ...
        s = ("[" + ("Agent" if not self.is_world else "World") + "]"
             + f" {self.node_name} (role: {self.node_profile.get_dynamic_profile()['connections']['role']})")
